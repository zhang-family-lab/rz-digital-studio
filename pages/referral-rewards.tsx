...
export default function ReferralRewards() {
    ...
    if (email) {
      ...
      try {
        await fetch('/api/capture-lead', {
          ...
          body: JSON.stringify({ email, honeypot, source: 'Referral Rewards Portal' }),
        });
      } catch (error) {
  ...
  if (!isAccessGranted) {
    return (
      <>
        <SeoHead 
          title="Referral Portal | RZ Digital Studio" 
          description="Access the RZ Digital Studio Referral Portal."
          noIndex={true}
        />
        <Header />
        <main className="bg-slate-50 min-h-screen flex items-center justify-center px-4">
          <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold text-slate-900">Referral Portal</h1>
              <p className="text-slate-500 text-sm mt-2">Enter your email to view the referral rewards program.</p>
            </div>
            ...
            <form onSubmit={handleAccess} className="space-y-4">
              ...
              >
                {loading ? 'Processing...' : 'Access Program'}
              </button>
            </form>

            <p className="text-center text-xs text-slate-400 mt-8">
              If you are a student or parent, please return to the <Link href="/" className="text-indigo-600 hover:underline">homepage</Link>.
            </p>
          </div>
        </main>
        <Footer />
